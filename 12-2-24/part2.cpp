...
int main() {
  ...
  while (getline(reportsStream, line)) {
    ...
    int isFixed = 0;
    for (int i = 0; i < reportLine.size(); i++) {
      vector<int> tmp = reportLine;
      tmp.erase(tmp.begin()+i); // erase each element one at a time
      isFixed = checkReport(tmp, positiveIncrements,negativeIncrements);
      if (isFixed) {
        break;
      }
    }
